...
module GraphRewriting.Graph.Write.Unsafe (module GraphRewriting.Graph.Write.Unsafe, module GraphRewriting.Graph.Types)
where
...
unregister (Node n) es = modifyEdgeMap $ flip (foldr $ Map.update deleteN) (map eKey es)
 where
  deleteN ns = if ns ≡ Set.singleton n then Nothing else Just $ Set.delete n ns

register :: Node -> [Edge] -> Rewrite n ()
register (Node n) es = modifyEdgeMap $ flip (foldr $ Map.alter addN) (map eKey es)
 where
  addN ns = Just (Set.insert n $ fromMaybe Set.empty ns)
